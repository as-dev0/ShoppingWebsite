...
function CheckoutPage() {
    ...
    const loadBillingAddresses = async () => {
        ...
        console.log("billing addresses are");
    ...
    const checkout = async () => {
        ...
        const orderResponse = await fetch(`/api/orders/${userid}` , {
            ...
            body: JSON.stringify({ userid:userid, 
                total:total, 
                date:Date().toLocaleString().slice(0,16),
                billingid: selectedBillingAddress,
                shippingid: selectedShippingAddress,
        ...
        // post order items
        for (let i = 0; i < cartItems.length; i++){
            await fetch(`/api/orderitem/${userid}/${orderid}` , {
                method: 'POST',
                body: JSON.stringify({ orderid:orderid, productid:cartItems[i].productid, quantity:cartItems[i].quantity}),
                headers: {'Content-Type': 'application/json',},
            });
        }
        ...
        // zero out cart
        for (let i = 0; i < cartItems.length; i++){
            fetch(`/api/cartitem/${userid}/${cartItems[i].productid}` , {
                method: 'DELETE',
                headers: {'Content-Type': 'application/json',},
            });
        }
    ...
    return (
        <>
            ...
            <div className='left'>
                ...
                {ShippingDropdown}
                <button onClick={toggleShippingOverlay}>Enter new shipping address</button>

                {BillingDropdown}
                <button onClick={toggleBillingOverlay}>Enter new billing address</button>

                {PaymentDropdown}
                <button onClick={togglePaymentOverlay}>Enter new credit card</button>
            ...
            <CheckoutProductList cartItems={cartItems} total={total} />
